# -*- coding: utf-8 -*-
"""JAOUHAR_MOHAMMED_Assignment1_Part2_NLTK.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1IUFk4-KmuEJewQXKx1sSZjdx2P7pHzPf
...
from nltk.probability import FreqDist # Added this import
...
def question_five():
    ...
    for word in moby_tokens:
        if word.isalpha() and len(word) > max_length:
            longest_word = word
            max_length = len(word)
...
def question_six():
    ...
    for word, freq in fdist.items():
        if freq > 2000:
            filtered_words.append((freq, word))
    return sorted(filtered_words, key=lambda x: x[0], reverse=True)
...
def question_eight():
    ...
    filtered_tokens = [
        token.lower() for token in moby_tokens
...
def question_nine():
    ...
    for token in moby_tokens:
        if token.isalpha():
            stemmed_tokens.append(porter.stem(token.lower()))
...
def question_ten():
    ...
    # 5. Lemmatize
    # Using 'v' for verb lemmatization, consistent with example_three.
